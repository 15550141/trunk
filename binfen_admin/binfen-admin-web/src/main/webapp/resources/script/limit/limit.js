...
jQuery(document).ready(function () {
    ...
    $("#searchSupply").click(function () {
        if (isBlank($("#supplierCode").val())) {
            $("#err-message").removeClass("alert-info").addClass("alert-error");
            $("#err-span").html("供应商简码为空");
            $("#err-message").show();
            return false;
        } else {
            $('#form1').attr("action", $("#searchSupplyAction").val());
            $('#form1').submit();
        }
    ...
    $("#apply").click(function () {
        ...
        if (isBlank($("#amount").val())) {
            $("#err-message").removeClass("alert-info").addClass("alert-error");
            $("#err-span").html("申请额度为空");
            $("#err-message").show();
            return false;
        } else {
            $("#limit").val($("#amount").val());
        }
        if (isBlank($("#firstAudit").val())) {
            $("#err-message").removeClass("alert-info").addClass("alert-error");
            $("#err-span").html("审核人ERP账号为空");
            $("#err-message").show();
            return false;
        }
        $('#form1').submit();
