...
function checkPrice(num,maxNum) {
	var array=new Array('yes','非法数字!','保留2位小数!','必须小于等于:'+maxNum+' ','不能位负数!');
	if(isNaN(num)){
		return array[1];
	}
    if(parseFloat(num)<0){
...
function checkPrice2(num,maxNum) {
	var array=new Array('yes','非法数字!','保留2位小数!','必须小于:'+maxNum+' ','不能位负数!');
	if(isNaN(num)){
		return array[1];
	}
    if(parseFloat(num)<0){
...
function checkPrice3(num) {
        	var array=new Array('yes','退款金额输入格式不合法!','保留2位小数!','不能为负数!');
        	if(isNaN(num)){
        		return array[1];
        	}
            if(parseFloat(num)<0){
